...
fn tonemap_pixel(r: f32, g: f32, b: f32, l_src: f32) -> (f32, f32, f32) {
    ...
    let desat_factor = 0.5 * (hdr_excess / (hdr_excess + 1.0));
...
mod tests {
    ...
    fn hue_preserved_through_luminance_tonemap() {
        ...
        assert_eq!(p[1], 165, "green channel must be desaturated to 165");
        assert_eq!(p[2], 165, "blue channel must be desaturated to 165");
    }
